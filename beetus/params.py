...
class SnakeParams:
    ...
    text_death: List[str] = field(default_factory=lambda: ["GAME OVER", "(Q)UIT", "(R)ESTART"])  # The text displayed on the death screen.
    ...
    height_score: int = pyxel.FONT_HEIGHT  # The height of the score bar at the top of the screen.
    ...
    down: Point = Point(0, 1)  # A Point object representing the down direction.
    right: Point = Point(1, 0)  # A Point object representing the right direction.
    left: Point = Point(-1, 0)  # A Point object representing the left direction.
    start: Point = Point(5, 5 + pyxel.FONT_HEIGHT)  # The starting position of the snake.
